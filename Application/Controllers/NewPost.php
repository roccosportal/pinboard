    ...
    public function indexAction() {
        ...
        if ($this->request->isPOST('submit')) {
            ...
            if (!$name || empty($name)) {
                ...
                $validationState->setError('Name', \Pvik\Core\Config::$config['Translations']['NewPost']['ErrorFieldTooShort']);
            }

            if (!$text || empty($text)) {
                $validationState->setError('Text', \Pvik\Core\Config::$config['Translations']['NewPost']['ErrorFieldEmpty']);
            } else if (strlen($text) < 20) {
                $validationState->setError('Text', \Pvik\Core\Config::$config['Translations']['NewPost']['ErrorFieldTooShort']);
            }
